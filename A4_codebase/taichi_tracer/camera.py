...
class Camera:
    ...
    def generate_ndc_coords(
        ...
        if jitter:
            # place the ray in a random location in the pixel using random()
            ndc_x = (((float(pixel_x) + ti.random()) / self.width) * 2) - 1
            ndc_y = (((float(pixel_y) + ti.random()) / self.height) * 2) - 1
        else:
            # center the ray in the pixel by adding 0.5
